...
| `/login/<provider>` | Connecte un utilisateur avec le provider <provider> . En mobile il faut appeler la route avec la querystring `auth_client=mobile` afin que la route `/autorize` renvoie le token JWT en querystring                                                                                                                       | Optionnel({user,password}) | {user,expires,token} ou redirect |
...
| `/authorize`        | Connecte un utilisateur à l'aide des infos retournées par le fournisseurs d'identités (Si redirection vers un portail de connexion par /login). Si la route `/login` a été appelée avec `auth_client=mobile`, la route redirige vers un scheme particulier contenant le token : `mobileapp://auth-callback?token=<token>` | {data}                     | redirect                         |
